...
public class WriteCdcToTable {
    String paimonWarehouse = "file:/tmp/paimon_test";
...
//        DataStream<RichCdcRecord> dataStream = env.fromElements(
//                        RichCdcRecord.builder(INSERT)
//                                .field("order_id", DataTypes.BIGINT(), "123")
//                                .field("price", DataTypes.DOUBLE(), "62.2")
//                                .build(),
...
//                                .build());
        ...
        DataStream<RichCdcRecord> dataStream2 =env.addSource(new SourceFunction<RichCdcRecord>() {
            @Override
            public void run(SourceContext<RichCdcRecord> ctx) throws Exception {
                Random random = new Random();
                for (long i = 0; i < 1000; i++) {
                    RichCdcRecord cdcRecord = RichCdcRecord.builder(INSERT)
                            .field("order_id", DataTypes.STRING(), "oid_" + (i/5*5))
                            .field("price", DataTypes.DOUBLE(),  String.valueOf(50 + random.nextInt(45)))
                            .field("dt", DataTypes.TIMESTAMP(), "2023-06-12 20:21:12")
                            .build();
                    ctx.collect(cdcRecord);
                    Thread.sleep(1000);
                }
            }
            @Override
            public void cancel() { }
        }).name("myTradeDetailStreamSrc");



        CatalogContext context = CatalogContext.create(new Path(paimonWarehouse));
        Catalog catalog = CatalogFactory.createCatalog(context);
...
//        catalogOptions = context.options();
    ...
    public void testReadPaimon() throws Exception {
        ...
        tableEnv.executeSql("CREATE CATALOG paimon WITH ('type' = 'paimon', 'warehouse'='" + paimonWarehouse + "')");
        tableEnv.executeSql("USE CATALOG paimon");
        // convert to DataStream
        org.apache.flink.table.api.Table table = tableEnv.sqlQuery("SELECT * FROM my_db.test_order");
