...
DROP TEMPORARY TABLE IF EXISTS dwd_trade_detail;
CREATE TEMPORARY TABLE dwd_trade_detail (
    trade_id BIGINT,
    trade_type INT,
    branch_id INT,
...
-- 3.1 Temporal Join , 动态拉链关联维表,丰富字段; 
DROP TABLE IF EXISTS dwd_trade_branch_info;
CREATE TABLE dwd_trade_branch_info (
trade_id BIGINT,
trade_type INT,
branch_id INT,
trade_amount DOUBLE,
...
INSERT INTO dwd_trade_branch_info 
SELECT
    dwd.* , dim.branch_name, dim.branch_type, dim.province, dim.branch_manager,
    timestampDiff(SECOND, dwd.trade_time, dim.update_time) AS timediff_sec
FROM dwd_trade_detail AS dwd
